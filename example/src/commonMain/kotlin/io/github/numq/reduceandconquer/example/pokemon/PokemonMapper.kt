...
fun Pokemon.toJson() = PokemonJson(
    id = id,
    name = PokemonJson.Name(english = name),
    types = types.map { it.name.lowercase().replaceFirstChar(Char::titlecase) },
    attributes = PokemonJson.Attributes(
        hp = attributes.hp.value,
        speed = attributes.speed.value,
        basicAttack = attributes.basicAttack.value,
        basicDefense = attributes.basicDefense.value,
        specialAttack = attributes.specialAttack.value,
        specialDefense = attributes.specialDefense.value
    )
)